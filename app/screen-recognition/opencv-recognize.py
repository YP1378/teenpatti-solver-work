...
def configure_opencv_runtime():
    ...
    if cuda_module is not None:
        try:
            cuda_device_count = int(cuda_module.getCudaEnabledDeviceCount())
    ...
    return {
        ...
        "optimized": True,
        "openclAvailable": opencl_available,
        "openclEnabled": opencl_enabled,
        "cudaAvailable": cuda_available,
        "cudaDeviceCount": cuda_device_count,
    }


RUNTIME_ACCELERATION = configure_opencv_runtime()
TEMPLATE_BANK_CACHE = {}
...
def build_mask_descriptor(mask):
    normalized_mask = np.where(mask > 0, 255, 0).astype(np.uint8)
    binary = normalized_mask > 0
    ...
    if area <= 0:
        return {
            ...
            "binary": binary,
            "area": 0,
            "projection_x": np.zeros((normalized_mask.shape[1],), dtype=np.float32),
            "projection_y": np.zeros((normalized_mask.shape[0],), dtype=np.float32),
            "contour": None,
        }

    contours, _ = cv2.findContours(normalized_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    contour = max(contours, key=cv2.contourArea) if contours else None
    return {
        "mask": normalized_mask,
        "binary": binary,
        "area": area,
        "projection_x": binary.mean(axis=0).astype(np.float32),
        "projection_y": binary.mean(axis=1).astype(np.float32),
        "contour": contour,
    }


def contour_similarity(candidate_descriptor, template_descriptor):
    candidate_contour = candidate_descriptor.get("contour")
    template_contour = template_descriptor.get("contour")
    if candidate_contour is None or template_contour is None:
        return 0.0
    distance = cv2.matchShapes(candidate_contour, template_contour, cv2.CONTOURS_MATCH_I1, 0.0)
    return clamp01(1.0 / (1.0 + (distance * 4.0)))


def projection_similarity(candidate_projection, template_projection):
    return clamp01(1.0 - float(np.mean(np.abs(candidate_projection - template_projection))))


def mask_similarity(candidate_descriptor, template_descriptor):
    candidate = candidate_descriptor["binary"]
    template = template_descriptor["binary"]
    intersection = float(np.logical_and(candidate, template).sum())
    union = float(np.logical_or(candidate, template).sum())
    candidate_area = float(candidate_descriptor["area"])
    template_area = float(template_descriptor["area"])
    ...
    projection_x = projection_similarity(candidate_descriptor["projection_x"], template_descriptor["projection_x"])
    projection_y = projection_similarity(candidate_descriptor["projection_y"], template_descriptor["projection_y"])
    shape = contour_similarity(candidate_descriptor, template_descriptor)
...
def template_entry(path, label, symbol_type, source):
    ...
    descriptor = build_mask_descriptor(canvas)
    return {
        "label": label,
...
def build_template_bank_cache_key(config):
    return json.dumps(
        {
            "rankTemplatesDir": normalize_cache_path(config.get("rankTemplatesDir")),
            "suitTemplatesDir": normalize_cache_path(config.get("suitTemplatesDir")),
            "builtinFontTemplateRoot": normalize_cache_path(config.get("builtinFontTemplateRoot")),
        },
        ensure_ascii=False,
        sort_keys=True,
    )
...
def build_symbol_candidates(card_image, base_region, symbol_type, family_hint):
    ...
    for variant_name, variant_mask in build_mask_variants(search_image):
        ...
        if signature in seen:
            continue
        seen.add(signature)
        normalized_mask = fit_mask_to_canvas(tight_mask, RANK_CANVAS if symbol_type == "rank" else SUIT_CANVAS)
        candidates.append({
            "variant": variant_name,
            "mask": normalized_mask,
            "descriptor": build_mask_descriptor(normalized_mask),
            "bbox": {
...
def match_symbol(candidates, templates, symbol_type, family_hint):
    ...
    for candidate in candidates:
        for template in templates:
            score = mask_similarity(candidate["descriptor"], template["descriptor"])
            if symbol_type == "suit":
...
def build_template_banks(config):
    cache_key = build_template_bank_cache_key(config)
    cached = TEMPLATE_BANK_CACHE.get(cache_key)
    ...
    banks = {"rank": rank_templates, "suit": suit_templates}
    TEMPLATE_BANK_CACHE[cache_key] = banks
    return banks


def recognize_config_payload(config, screenshot):
...
def recognize_payload(payload):
    ...
    if isinstance(jobs, list) and jobs:
        ...
        for index, job in enumerate(jobs):
            config = job["config"]
            screenshot_path = job.get("screenshotPath") or payload.get("screenshotPath")
            if not screenshot_path:
                raise RuntimeError(f"Missing screenshotPath for batch job {index}.")
            screenshot = screenshot_cache.get(screenshot_path)
            if screenshot is None:
                screenshot = read_image(screenshot_path)
                screenshot_cache[screenshot_path] = screenshot
            result = recognize_config_payload(config, screenshot)
            result["jobId"] = job.get("jobId", index)
    ...
    return recognize_config_payload(config, screenshot)
